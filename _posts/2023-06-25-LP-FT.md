...
이전에는 데이터 셋의 규모가 작기 때문에, 특정 task들에 대해서는 transfer learning을 하는 것이 오히려 좋지 않은 영향을 주기도 하였고 많은 경우에 ImageNet 정도 사이즈의 데이터 셋에서 학습한 pretraiend model의 parameter를 가져와서 이보다도 더 작은 downstream task로 fine-tuning 하였었습니다. 
적은 데이터 셋에서 밑단부터 시작하여 학습
