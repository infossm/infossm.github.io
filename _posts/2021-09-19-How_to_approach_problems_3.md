...
허나 관찰에서 다루었던 것과 격자가 달라졌기 때문에, 누적합을 적용할 때에 직사각형의 $x$축 좌표들과 $y$축 좌표들이 바로 알아낼 수는 없다는 문제가 생깁니다.
이를 해결하기 위해서 two pointer 방법을 사용하여, 직사각형 x축 변으로 사용할 수 있는 구간들과 직사각형의 $y$축 변으로 사용할 수 있는 구간들을 구해놓는 방식으로 전처리를 해주거나 누적합을 구하는 과정에서 수행해줄 필요가 있습니다.
