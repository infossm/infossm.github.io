...
$$
A = \sum_{i=1}^{\infty} {1 \over {2^i}} V_{a_i}  \\ (a_i = 1,2,3), \ \ ( V_1, V_2, V_3) = ({1 \over \sqrt3}i,\ {1 \over 2} -{1 \over {2\sqrt3}}i,\ -{1 \over 2} -{1 \over {2\sqrt3}}i )
$$
...
이제 실제 유클리디안 거리는 $|D| = \sqrt{DD^*}$으로 표현할 수 있고, 지애 상수는 확률 변수 $X_1, X_2, ...$에 대해 결정되는 $\sqrt{DD^*}$의 값의 기댓값이라고 할 수 있습니다.



#### How to get $|D|$?

드디어 문제 정의를 마쳤습니다. 이제 이렇게 정의된 $|D|=\sqrt{DD^*}$의 기댓값 $E[\sqrt{DD^*}]$을 어떻게 정리해서 구할 지 알아봅시다.

현재 기댓값을 구하는 데 있어 가장 큰 걸림돌은 식에 씌워진 루트라고 할 수 있겠습니다. 실제로 루트를 제거한 $E(DD^*)$의 경우 $2/9$로 깔끔한 값을 가집니다. 그럼 어떻게 수식에서 루트를 떼어낼 수 있을까요?

이는 **테일러 전개**에서 힌트를 얻을 수 있습니다. 테일러 전개에 의하면, $\sqrt{1+X}$는 아래와 같이 나타낼 수 있습니다.
$$
\sqrt{1+X} = 1 + {1\over 2}X - {1\over8}X^2 + {1\over 16}X^3 - \cdots
$$
즉, $\sqrt{F}$ 꼴의 식을 $F$의 polynomial한 식으로 바꿔서 전개를 할 수 있게 됩니다. 이 문제의 경우 $E[D^p {D^*}^q ]$의 값을 꽤 쉽게 구할 수 있기 때문에(후술됩니다), 이런 식의 전개가 매우 유효한 접근이라고 할 수 있겠습니다.



#### $E[D^p {D^*}^q]$ 구하기

전개된 테일러 식으로 지애 상수를 구하기 위해서는, 전개된 각 $E[D^p{D^*}^q ]$의 값들을 알 수 있어야 합니다. 이를 구하기 위해 다음 4개의 property가 사용됩니다.

(0) $E[D^0{D^*}^0]=1$입니다.

​	proof) 자명합니다.

(1) $6 \nmid p-q$면, $E[D^p {D^*}^q ] = 0$이 성립합니다.

​	proof) $D$에서 각각의 확률 변수 $X_i$는 60도 대칭임을 관찰하면, $E[f(D)] = E[f(e^{{\pi i \over 3}} D)]$임을 관찰할 수 있습니다. 따라서 $E[D^p {D^*}^q ] = E[(e^{{\pi i \over 3}} D)^p {(e^{{\pi i \over 3}} D)^*}^q ] = E[D^p {D^*}^q ] e^{{(p-q)\pi i \over 3}}$이 성립하며, 가정에 의해 $e^{{(p-q)\pi i \over 3}} \neq 1$이므로 $E[D^p {D^*}^q ] = 0$입니다.

(2) $6 \mid p - q$면, $b = ae^{{\pi i} \over 3}$에 대해 $E[(a+D)^p({a^*} + D^*)^q] = E[(b+D)^p({b^*} + D^*)^q]$가 성립합니다.

​	proof) (1)의 증명과 유사하게, $E[f(D)] = E[f(e^{{\pi i \over 3}} D)]$가 성립하므로 
$$
E[(b+D)^p({b^*} + D^*)^q] = E[(ae^{{\pi i} \over 3}+De^{{\pi i} \over 3})^p({a^*}e^{-{{\pi i} \over 3}} + D^*e^{-{{\pi i} \over 3}} )^q]\\ = e^{{(p-q)\pi i} \over 3}E[(a+D)^p({a^*} + D^*)^q] = E[(a+D)^p({a^*} + D^*)^q]
$$
​	이 성립합니다.
...
위 property들로 $E[ D^p{D^*}^q ]$의 값의 점화식을 유도해봅시다. property (2), (3)을 사용하면, $6 \mid p-q$일 때, $E[D^p {D^*}^q]$는 아래와 같이 표현됩니다.
$$
E[D^p {D^*}^q] = \sum_{x_i\in X_1} p(x=x_i)E[(x_i + {1 \over 2}D)^p (x_i^* + {1 \over 2}D^*)^q ]
\\ = {1\over3}E [ (0 + {1\over 2}D)^p(0 + {1 \over 2}D^* )^q ] + {2 \over 3}E [ ({1\over2} + {1\over 2}D)^p({1\over2} + {1 \over 2}D^* )^q ]
$$


이를 통해 $E[D^i{D^*}^j]$의 점화식을 구할 수 있습니다. 
$$
E[D^p {D^*}^q] = 
\begin{cases}
{2\over3} {1\over {2^{p+q}-1}} \sum_{i=0}^p \sum_{j=0}^q d_{p,q}(i,j) \times {_pC_i} {_qC_j} E[D^i {D^*}^j] & ( 6 \mid p-q )\\
\phantom{11111111111111111111}0 & \text{otherwise}
\end{cases}
$$
(이때 $d_{p,q}(i, j) = 0 \ \text{if} \ (i,j)=(p,q),\ 1 \ \text{otherwise}$)

이를 이용해 이전에 언급한 $E(DD^*)$의 값을 구해보면,
$$
E[D^1 {D^*}^1] = {2\over3} {1\over {2^{2}-1}} \sum_{i=0}^p \sum_{j=0}^q d_{p,q}(i,j) = {2\over9}(E[D^0{D^*}^0]+ E[D^0{D^*}^1] + E[D^1{D^*}^0]) = {2 \over 9}
$$
임을 확인할 수 있습니다.

저 점화식의 꼴을 그대로 사용하면 $E[D^n{D^*}^n]$를 구하기 위해 $O(n^2)$의 공간복잡도와 $O(n^4)$의 시간복잡도가 필요하며, 간단한 DP 최적화를 거치면 시간복잡도 $O(n^3)$에 이를 구할 수 있게 됩니다. 지애 상수의 222자리까지 구하기 위해서는 대략 $n=1000$까지의 $E[D^n {D^*}^n]$이 필요하며(로직 및 구현 방식에 따라 차이가 있을 수 있습니다), 두 버전 모두 충분히 빠른 시간 내에 이들을 구할 수 있습니다.
...
위 단계에서 각각의 $E[D^p {D^*}^q]$를 구하기는 했으나, 당장 $E[ \sqrt{DD^*} ]$을 가지고 테일러 전개를 하는 것은 쉽지 않습니다. 단순히 $DD^* = 1 + (DD^* - 1) = 1+x$로 넣고 계산을 시도해보면, 테일러 급수의 초기 100개의 항을 더하면 `1e90`이라는 놀라운 값으로 날뛰는 걸 확인해볼 수 있습니다. 상식적으로 지애 상수가 저 값으로 수렴할 리는 없습니다. 어디서 문제가 발생한 걸까요?

이는 $x=\sqrt{DD^*}-1$의 값이 $[-1,0]$ 사이의 값을 가지고, 따라서 $\sqrt{1+x}$의 수렴반경 $(-1, 1)$을 벗어났기 때문입니다. 물론, $x=-1$이 되는 경우는 $D=0$인 경우 뿐이므로 배제를 할 수 있습니다. 하지만 실질적인 문제는 $|D|$의 값이 매우 작을 때 발생합니다.

먼저 다음 lemma가 성립합니다. 계산 과정은 생략하겠습니다.

​	Lemma) $\sqrt{1+x}$의 테일러 급수의 $k$번째 항까지 계산한 후 남은 오차 $R_k(x)$는 $O(|x|^{k+1})$이다.

이제 적당히 작은 실수 $h>0$에 대해, $|D|<h$인 케이스를 생각해봅시다. $D$가 이런 값을 가질 확률을 $p_h$라 할 때, $p_h > 0$임을 쉽게 관찰할 수 있습니다($D$의 초기 $r$개의 $X_i$ 값이 $0$이 되는 케이스를 생각해보면 됩니다). 이러한 $D$에 대해 $x = \sqrt{DD^*}-1 < -(1-h)$이며, lemma에 의해 테일러 급수의 앞 $k$개의 항을 계산했을 때 실제 지애 상수 값과의 오차는 $O((1-h)^k)$임을 알 수 있습니다. 만약 $h={1\over k}$로 잡으면 위 오차는 대략 $1/e$의 스케일이 될 것이고, 이는 $k$개의 급수 연산으로는 오차가 충분히 좁혀지지 않음을 의미합니다. 정리하자면, 아무리 큰 $k$를 잡고, $x=DD^*-1$로 계산한 테일러 급수 중 앞 $k$개를 계산한다고 해도, $h={1\over k}$에 대해 $p_h$ 확률로 등장하는 $D$의 원소들은 충분히 수렴하지 못하기에 이 방법으로는 원하는 수렴값을 얻을 수 없습니다. 
...
$$
마지막 식에서 $|1+{1\over2}x_i|$의 값들은 모두 $|{3 \over 4} + {\sqrt{3} \over 4}i| \approx 0.866 $보다 크며, 각 $x_i$마다 식정리를 하면 $E[|1+UD|] = E[\sqrt{(1+UD)(1+U^*D^*)} ]$, $|UD| < 0.7$ 꼴의 식이 나오게 됩니다. 따라서 각 $\sqrt{1+UD}$, $\sqrt{1+U^*D^*}$의 테일러 급수는 빠르게 수렴함이 보장되고, 각 식을 펼치면 나오는
$$
E[\sqrt{1+UD} \sqrt{1+U^*D^*} ] \\
= E[(1 + \frac{1}{2}UD - \frac{1}{8} (UD)^2 + \cdots)(1 + \frac{1}{2}U^*D^* - \frac{1}{8} (U^*D^*)^2 + \cdots) ] \\
= E[1] + {1\over 2}UE[D] + {1\over 2}U^*E[D^*] + {1\over4}UU^*E[DD^*] + \cdots
$$
