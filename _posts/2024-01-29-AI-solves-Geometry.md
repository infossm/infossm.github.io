...
이 문제는 2020년 1번으로 출제된 문제입니다. AlphaGeometry는 3개의 보조점을 잡아 35줄의 풀이를 완성하였습니다. 그런데 풀이를 읽어보면, 보조점의 직관성이 떨어질뿐더러 풀이가 지나치게 복잡한 감이 있습니다. 실제로 이 문제는 세 직선의 실제 교점으로 추정되는 <삼각형>의 외심을 잡으면 바로 쉽게 풀립니다. AlphaGeometry가 이러한 접근을 찾지 못한 점에 대해 두 가지 요인이 있다고 생각됩니다.
