...
첫 번째 줄에서는 $1$바이트씩 숫자를 이어붙입니다. 각 $1$바이트에 저장된 값을 순서대로 $d_0, d_1, \cdots, d_7$이라 합시다. 인접한 두 값 $(d_2i, d_{2i+1})$은 <code>(x * 10) + (x >> 8)</code>에서 $(10d_2i + d_{2i+1}, *)$이 되고, <code>0x00ff</code>와 bitwise and 연산을 통해 $(10d_2i + d_{2i+1}, 0)$이 됩니다. 따라서 첫 번째 줄이 실행되면 $x$의 각 $2$바이트에 두 자리 숫자가 저장됩니다.

비슷하게 두 번째 줄에서는 $2$바이트씩 숫자를 이어붙입니다. 인접한 두 값 $(d_2i, d_{2i+1})$은 <code>(x * 100) + (x >> 16)</code>에서 $(100d_2i + d_{2i+1}, 0)$이 되고, <code>0x0000ffff</code>와 bitwise and 연산을 통해 $(100d_2i + d_{2i + 1}, 0)$이 됩니다.
...
사용 예시는 다음과 같습니다. [(코드)](http://boj.kr/a52d3ca0103c4e2da98bff3e19104fea)
...
사용 예시는 다음과 같습니다. [(코드)](http://boj.kr/45d106fcfe154edb939df009e7d7f5ad)
...
사용 예시는 다음과 같습니다. [(코드)](http://boj.kr/cab4339ac93d4050beac27a734b0e19a)
