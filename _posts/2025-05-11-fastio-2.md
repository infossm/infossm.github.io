...
## Faster I/O Implementation

지금까지 <code>read</code> 또는 <code>mmap</code>을 이용한 빠른 입력 방식과 <code>write</code>를 이용한 빠른 출력 방식을 살펴보았습니다. 이 방식은 입력과 출력 버퍼를 두고 문자를 하나씩 처리하며, 버퍼가 가득 차면 한 번에 입출력을 수행하는 구조입니다. 대부분의 문제에서는 이 정도 최적화만으로 충분합니다. 다만, 한 번에 여러 문자를 처리할 수 있다면 입출력 속도를 더욱 높일 수 있습니다.

### Faster Input using Bit-Twiddling Hacks
...
코드에서 <code>\*(u64\*)pr</code>는 <code>pr</code>를 $64$비트 정수 자료형의 포인터로 형변환하여 역참조하며 <code>pr[0]</code>, $\cdots$, <code>pr[7]</code>을 $64$비트 정수 자료형 <code>x</code>에 저장합니다.
...
첫 번째 줄에서는 $1$바이트씩 숫자를 이어붙입니다. 각 $1$바이트에 저장된 값을 순서대로 $d_0, d_1, \cdots, d_7$이라 합시다. 인접한 두 값 $(d_i, d_{i+1})$은 <code>(x * 10) + (x >> 8)</code>에서 $(10d_i + d_{i+1}, *)$이 되고, <code>0x00ff</code>와 bitwise and 연산을 통해 $(10d_i + d_{i+1}, 0)$이 됩니다. 따라서 첫 번째 줄이 실행되면 $x$를 $2$바이트씩 끊어서 읽을 때 각 $2$바이트에 두 자리 숫자가 저장되게 됩니다.
...
이후 아직 처리되지 않은 글자를 위와 같이 한 글자씩 순차적으로 읽으며 처리하면 전체 입력을 처리할 수 있습니다.
...
### Faster Input using Bit-Twiddling Hacks 2
...
### Faster Output using Pre-Calculation
