...
<code>std::bit_width</code>는 <code>bit</code> 헤더에 정의된 함수로 $\lceil\log_2(n+1)\rceil = \lfloor\log_2n\rfloor+1$을 반환합니다. 여기에 $\log_{10} 2 = 0.301029995664\dots \approx \frac{1233}{4096}$를 곱하면 <code>std::bit_width(n) * 1233 >> 12</code>로 $\lfloor\log_{10} n\rfloor$을 근사할 수 있습니다.
...
따라서 $1 \leq n < 2^{680}$ 범위에서는 $0 \leq e \leq 1$이 성립하고, 128bit 이하 모든 unsigned integer에 대해 <code>std::bit_width(n) * 1233 >> 12</code>는 $\lfloor\log_{10} n\rfloor$ 또는 $\lfloor\log_{10} n\rfloor + 1$입니다.

$10^0, 10^1, \cdots$를 미리 구해두면 로그 값이 $1$ 크게 구해진 경우 오차 보정을 해줄 수 있고, $\lfloor\log_{10}n\rfloor + 1$을 구해 리턴할 수 있습니다.
