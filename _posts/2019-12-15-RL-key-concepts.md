...
* 에이전트가 어떤 행동을 했을 때 따라오는 이득을 **보상(Reward)** 이라고 합니다. 말 그대로 보상이기 때문에 높을수록 좋습니다. 보상값에 -1을 곱해서 비용(Cost)이라고 부르기도 하고, 이 경우는 낮을수록 좋습니다.
  ...
  상태 $$ s_t $$ 에서 행동 $$ a_t $$ 를 취해 상태가 $$ s_{t+1} $$ 로 바뀌면서 얻은 보상은 $$ r_t $$ 라고 하고[^1], 이를 나타내는 함수를 $$ R: S \times A \times S \rarr \mathbb{R} $$ 로 나타냅니다.
...
먼저 **몬테카를로(Monte Carlo)**  방식에 대해 알아보겠습니다. 위에서 살펴본 것처럼 $$ V_\pi(s)$$ 는 $$  E_\pi[G_t |S_t=s] $$ 와 동일한데, 이를 **샘플링**을 통해서 구해 $$ V_\pi(s)$$ 가 해당 값과 가까워지게 학습하는 방식을 통틀어서 몬테카를로 방법이라고 합니다. 즉 $$ \pi $$ 를 이미 알고 있을 때 에피소드의 처음부터 끝까지  $$ \pi $$ 를 따라 행동한 후, 수집한 데이터를 바탕으로 각 $$ s_t $$ 에 대응되는 누적 보상 $$ G_t $$ 를 구해서 $$ V_\pi(s_t) $$ 가 target value $$ G_t  $$ 와 가까워지게 업데이트 하는 것을 반복하는 것입니다. 
...
$$
위와 같이 식을 전개하면 $$ \pi $$ 에 대한 기댓값을 $$ \mu $$ 에 대한 기댓값으로 나타낼 수 있습니다. 이로써 과거 policy $$ \mu $$에서 샘플링한 action을 $$ \pi $$ 를 업데이트하는데 사용할 수 있게 됩니다. 바뀐 수식을 보면 $$\pi(a|s)$$ 와 $$ \mu(a|s) $$ 를 명시적으로 계산할 수 있어야 하는데, 다행히 대부분의 강화학습 세팅에서 우리는 policy를 계산할 수 있으므로 이 부분이 문제가 되지는 않습니다.
...
Q-learning 은 optimal policy의 q function (state-action value function)을 학습한 다음, 주어진 상태에 대해 q 값이 가장 큰 action을 고르는 방법을 policy로 가지는 강화학습 알고리즘입니다.

#### Tabular Case

Tabular는 함수를 표(table)로 나타낸다는 말인데, 가능한 모든 함수 입력값에 대해 결과값을 따로 정확하게 관리할 수 있는 경우를 뜻합니다. 이론적으로는 너무 당연한 말인데 굳이 따로 지칭하는 이유는, 실제로 알고리즘을 구현할 때는 이것이 불가능한 경우가 많기 때문입니다. 예를 들어 가능한 state와 action이 각각 10만개라면 (state, action)의 조합은 100억개가 될텐데 모든 조합에 대한 함수 결과값을 메모리에 저장하는 것은 거의 불가능할 것입니다. 정리하자면 tabular case라고 하면 메모리 용량에 제한이 없고, 함수값 갱신(대입)도 제약없이 가능하다는 뜻이 됩니다.

이제 Tabular Case 에서의 Q-learning 알고리즘을 알아보겠습니다.
...
앞서 살펴본 Q-learning은 optimal policy 자체를 학습하는 것이 아니라, optimal q function을 학습한 후 optimal policy를 유도하는 우회책을 사용했습니다. 그런데 직접적으로 policy를 개선하는 것은 왜 어려울까요?

policy의 성능을 평가하는 가장 쉬운 방법은 해당 policy $$ \pi $$ 를 따랐을 때 initial state $$s_0 $$ 의 value function $$ V(s_0) $$ 값을 보는 것입니다. 첫 상태에서 출발한 후로 얼마나 많은 보상을 모을 수 있는지 나타내는 값이므로 policy의 성능을 평가하기에 적합합니다. 따라서 policy parameter를 $$ \theta $$  라고 두면,  $$ \theta$$ 에 대한 $$ V(s_0) $$ 의 그래디언트를 계산하고 Gradient Ascent 알고리즘을 통해 $$ \theta $$ 를 업데이트하면 $$ \pi $$ 의 성능이 나아짐을 기대할 수 있을 것입니다.

그런데 $$ V(s_0) $$ 의 그래디언트를 계산하면 다음과 같습니다. 
$$
