...
$\forall a,b,c\in G: (a*b=a*c \rightarrow b=c) \and (b * a = c * a \rightarrow b = c)$

가 성립하므로, 임의의 ordinal $a' < a, b' < b$에 대하여 $a \oplus b \ne a' \oplus b$ 그리고 $a \oplus b \ne a \oplus b'$이 성립해야 합니다. 따라서

$a \oplus b = \mathrm{mex} \lbrace a' \oplus b' : (a' < a \and b' = b) \or (a' = a \and b' < b) \rbrace$
...
$(a \oplus b) \oplus c = \mathrm{mex} \lbrace d' \oplus c' : (d' < a \oplus b \and c' = c) \or (d' = a \oplus b \and c' < c)  \rbrace $

입니다.

$S = \lbrace d' \oplus c' : (d' < a \oplus b \and c' = c) \or (d' = a \oplus b \and c' < c)  \rbrace $

라고 하면 임의의 $(a \oplus b) \oplus c \notin T$인 $S \subseteq T \subset \Omega$에 대하여 $(a \oplus b) \oplus c = \mathrm{mex} \space T$임을 알 수 있습니다.

$S' = \mathrm{mex} \lbrace (a' \oplus b') \oplus c' : (a' < a \and b' = b \and c'  = c) \or (a' = a \and b' < b \and c' = c) \or (a' = a \and b' = b \and c' < c) \rbrace$ 
...
$a \oplus a = \mathrm{mex} \lbrace a' \oplus a'' : (a' < a \and a'' = a) \or (a' = a \and a'' < a) \rbrace$
...
$\begin{align} a \oplus b &= \mathrm{mex} \lbrace a' \oplus b' : (a' < a \and b' = b) \or (a' = a \and b' < b) \rbrace \newline &= \mathrm{mex} \lbrace b' \oplus a' : (a' < a \and b' = b) \or (a' = a \and b' < b) \rbrace \newline &= b \oplus a \end{align}$
...
$a \otimes b = \mathrm{mex} \lbrace (a \otimes b') \oplus (a' \otimes b) \oplus (a' \otimes b' ) : a' < a \and b' < b \rbrace$
